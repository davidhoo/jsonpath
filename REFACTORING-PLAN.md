...
- [ ] 1.1 创建新的目录结构
  - [ ] 创建 `pkg` 目录用于存放核心功能包
  - [ ] 创建 `internal` 目录用于存放内部助手功能
  - [ ] 保留 `cmd` 和 `examples` 目录

- [ ] 1.2 创建核心子包目录
  - [ ] 创建 `pkg/parser` 用于解析 JSONPath 表达式
  - [ ] 创建 `pkg/segments` 用于实现各种段
  - [ ] 创建 `pkg/functions` 用于实现 JSONPath 函数
  - [ ] 创建 `pkg/eval` 用于实现执行引擎
  - [ ] 创建 `pkg/compare` 用于比较和过滤功能
  - [ ] 创建 `pkg/errors` 用于错误处理

### 阶段二：核心接口和类型迁移

- [ ] 2.1 创建和迁移基础接口和类型
  - [ ] 创建 `pkg/segments/segment.go` 定义 segment 接口
  - [ ] 创建 `pkg/functions/function.go` 定义 Function 接口
  - [ ] 创建 `pkg/errors/errors.go` 定义错误类型和常量
